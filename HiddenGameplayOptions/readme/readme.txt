...
Because of the great number of available customizations options the mod components have been
split into several component groups.

A) *** Group: Game Options ***

1. Install all Hidden Gameplay Options at once

This is a convenience option that allows you to install all available game options at once.
The improved Cheat Menu and other GUI improvements are not included. See more detailed information
...
22. Add in-game option "Frame Rate" (experimental) (not available for PST:EE, not compatible with
...
You can choose a rate between 10 and 60 frames per second, in steps of 5. The game's default is 30 fps.
...
23. Add in-game option "Action Feedback" (not compatible with Lefreut's Enhanced UI (LeUI),
...
24. Add in-game option "Display Level Up Icon" (only available for PST:EE, requires "Level Up"
    Icon Tweaks)
...
31. Add in-game options for Tweak Anthology's "Create Interval Saves" (requires Tweaks Anthology
...
33. Add in-game option for Tweak Anthology's "Restored Loading Hints" (requires Tweaks Anthology
...
This option can be used to update the resource tables of the improved cheat menu which can be
helpful if other content mods are installed afterwards.
It isn't registered in the WeiDU.log and can therefore be executed as often as desired.


C) *** Group: GUI Improvements ***

This is a loose collection of minor improvements that focuses on various visual and usability
aspects of the GUI. They are currently only available for the default GUIs of the supported games.

1. Improved "Death Screen" options

The death screen is shown after the protagonist or the whole party is killed and provides options
to quit or load a game.
This component expands the dialog by the option to automatically load the last save if available.


2. Improved "Quit Game" options

Pressing Alt-F4 brings up a dialog with the options to quit the game or cancel the request.
This component expands the dialog by the option to go to the start screen of the game instead.


3. Game Menu: Fire animation over SoD logo (SoD and EET only)

The start screen of the game shows a nifty fire animation over the game logo if the "Siege of
Dragonspear" campaign is active.
This component adds the same fire animation to the options menu that can be brought up by pressing
the ESC key or Options button in a running game.


4. Game Menu: Improved "Quit Game" options (IWD:EE only)

Pressing "Quit Game" from the options menu in a running game provides the options "Cancel",
"Quit", and "Save" where the "Quit" option just returns to the start screen of the game.
This component changes the "Quit" option to quit the game completely and adds another option
"Main Menu" that returns to the start screen of the game instead.


5. Game Menu: Improved "Load Game" options

Pressing "Load Game" from the options menu in a running game provides the options "Cancel" and
