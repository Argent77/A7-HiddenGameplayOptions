...
Rekomendowana jest instalacja tej modyfikacji po Tweaks Anthology, jak również modyfikacjach modyfikujących
...
Ze względu na dużą liczbę opcji komponenty zostały podzielona na kilka kategorii.

A) *** Kategoria: Opcje gry ***

1. Zainstaluj wszystkie Ukryte opcje gry

Komponent umożliwia instalację wszytkich ukrytych opcji gry naraz. Pomija opcję "Ulepszone menu opcji ułatwiających grę" oraz inne ulepszenia interfejsu użytkownika. Szczegółowe informacje dotyczące poszczególnych opcji znajdują się poniżej.

Poniższe komponenty będą dostępne po pominięciu komponentu "Zainstaluj wszystkie Ukryte opcje gry":
...
22. Dodaj w grze opcję „Wskazówki dotyczące wykonywanych czynności” (niekompatybilne z modyfikacjami
    „Lefreut's Enhanced UI (LeUI)”, "K4f4r's Classic BG UI" i "Tipun's User Interface")
...
32. Dodaj w grze opcje przeznaczone dla komponentu "Przywrocone podpowiedzi na ekranach wczytywania"
    z modyfikacji Tweaks Anthology (wymaga uprzedniego zainstalowania komponentu "Przywrocone podpowiedzi na ekranach     wczytywania" z modyfikacji Tweaks Anthology)

Opcja ta włącza pokazywanie podpowiedzi z oryginalnej wersji Baldur's Gate II na ekranie dialogowym gry. Do jej poprawnego działania wymagane jest zainstalowanie komponentu "Przywrocone podpowiedzi na ekranach wczytywania" z modyfikacji Tweaks Anthology.

Opcja ta znajduje się w sekcji Opcje gry.


B) *** Kategoria: Menu opcji ulatwiajacych gre ***

1. Ulepszone menu opcji ułatwiających grę
...
2. Zaktualizuj tabele zasobów dla komponentu „Ulepszone menu opcji ułatwiających grę”
...
C) *** Kategoria: Ulepszenia UI ***

Jest to zbiór róznych niewielkich ulepszeń funkcjonalności i strony wizualnej interfejsu użytkownika. Na chwilę obecną są one dostępne tylko dla domyślnych interfejsów wspieranych gier.

1. Rozszerzenie opcji "Ekranu smierci"

Ekran śmierci pojawia się po śmierci protagonisty lub całej drużyny, oferując opcje zakończenia gry albo wczytania jej zapisu.
Ten komponent dodaje opcję automatycznego wczytania ostatniego zapisu, jeśli taki jest dostępny.


2. Rozszerzenie opcji "Wyjdz z gry"

Wciśnięcie w grze klawiszy Alt-F4 wywołuje okno z opcjami zakończenia gry i anulowania żądania.
Ten komponent dodaje do okna opcję wyjścia do menu głównego gry.


3. Menu gry: Animacja ognia w logo SoD (dostępne tylko dla SoD i EET)

W menu głównym gry widoczna jest ładna animacja ognia na logu dodatku Siege of Dragonspear, jeśli gracz uaktywnił tę kampanię.
Ten komponent dodaje taką samą animację ognia w menu opcji, które można wywołać przez wciśnięcie klawisza ESC lub wejście w nie przyskiem Opcje w trakcie gry.


4. Menu gry: Ulepszone opcje "Wyjdz z gry" (dostępne tylko dla IWD:EE)

Wybranie opcji "Wyjdź z gry" z menu opcji w otwartej grze oferuje możliwości: "Anuluj",
"Wyjdź" i "Zapisz", gdzie opcja "Wyjdź" cofa tylko do menu głównego gry.
Ten komponent zmienia funkcję opcji "Wyjdź", by powodowała zupełne wyjście z gry, a także dodaje kolejną opcję
"Menu główne", która cofa do menu głównego.


5. Menu gry: Ulepszone opcje "Wczytaj gre"

Wybranie opcji "Wczytaj grę" z menu opcji w otwartej grze oferuje możliwości: "Anuluj" i "Wczytaj".
Ten komponent dodaje trzecią opcję: kontynuownie ostatniego zapisu gry, jeśli taki jest dostępny.


D) *** Kategoria: Przypisanie klawiszy

1. Zaktualizuj przypisanie klawiszy dla czarów kapłańskich i/lub maga
